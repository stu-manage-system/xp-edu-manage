        ...
        <el-table-column prop="status" label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="getMemberStatusTag(row.status)">
              {{
                useMemberStore().getMemberStatus.find(
...
const submitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        const api = form.memberCode
          ? MemberApi.updateMember
          : MemberApi.addMember;
        const res = await api(
          form.memberCode ? form : _.omit(form, "memberCode")
        );
        if (res.code === 0) {
