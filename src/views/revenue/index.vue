            ...
            <div class="search-left">
              <el-row :gutter="24" class="search-row">
                ...
                <el-col :span="6" v-if="searchForm.businessCode">
                  <el-form-item
                    :label="
                      searchForm.businessCode === 'MEMBER_CARD'
                        ? '会员卡名称'
                        : '私教课名称'
                    "
                    prop="revenueType"
                  >
                    <el-select
                      v-model="searchForm.revenueType"
                      :placeholder="`请选择${searchForm.businessCode === 'MEMBER_CARD' ? '会员卡名称' : '私教课名称'}`"
                      clearable
                      class="w-full"
                    >
                      <el-option
...
const getUserList = async (pageNum = 1, pageSize = 10) => {
  ...
  const startTime = dateRange.value.length
    ...
    : "";
  const endTime = dateRange.value.length
...
const getCourseType = async () => {
  ...
  courseTypeOptions.value = res.data.filter(
