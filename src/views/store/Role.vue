  ...
  <div class="page-content">
    <el-row>
      <el-col :xs="24" :sm="12" :lg="6">
        <el-input v-model="searchForm.roleName" placeholder="部门名称" clearable></el-input>
      </el-col>
    ...
    <art-table :data="tableData" :total="total" :current-page="currentPage" :page-size="pageSize" :loading="isLoading"
      ...
      <template #default>
        ...
        <el-table-column fixed="right" label="操作" width="300" #default="scope">
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="角色类型" prop="roleType">
          <el-select v-model="form.roleType" placeholder="请选择角色类型">
            <el-option v-for="item in roleConfig" :label="item.label" :value="item.value" :key="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="数据类型" prop="menuType">
          <el-select v-model="form.menuType" placeholder="请选择数据类型">
            <el-option v-for="item in menuConfig" :label="item.label" :value="item.value" :key="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="form.remark" type="textarea" placeholder="请输入备注" />
        </el-form-item>
        <el-form-item label="配置菜单角色" v-if="form.menuType === 'CUSTOM' || form.menuType === '自定义菜单'" prop="menuIds">
          <el-tree-select ref="treeRef" v-model="form.menuIds" :data="roleTreeData" :render-after-expand="false"
            show-checkbox multiple placeholder="请配置菜单角色" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
        ...
        <template #default>
          <div class="permission-detail">
            <el-descriptions :column="2" border label-width="160">
              <el-descriptions-item label="角色名称" label-class-name="my-label">
                {{ form.roleName }}
              </el-descriptions-item>
              <el-descriptions-item label="角色类型" label-class-name="my-label">
                {{ form.roleType }}
              </el-descriptions-item>
              <el-descriptions-item label="数据类型" label-class-name="my-label">
                {{ form.menuType }}
              </el-descriptions-item>
              <el-descriptions-item label="备注" label-class-name="my-label">
                {{ form.remark }}
              </el-descriptions-item>
              <el-descriptions-item label="菜单权限" label-class-name="my-label">
                <el-tree ref="treeIdRef" :data="roleTreeData" :default-checked-keys="form.menuIds"
                  :default-expand-all="true" node-key="value" show-checkbox :props="{
                    children: 'children',
                    label: 'label',
                    disabled: 'disabled'
                  }" />
...
const getRoleList = async (pageNo = 1, pageSize = 10) => {
  ...
  const res = await UserService.queryStoreRoleList(data)
  if (res.code === 0) {
...
const getRoleDetail = async (roleId: string) => {
  const res = await UserService.getStoreRoleDetail(roleId)
  if (res.code === 0) {
...
const rules = {
  ...
  menuType: [{ required: true, message: "请选择数据类型", trigger: "blur" }],
    ...
    if (valid) {
      ...
      const res =
        dialogType.value === "add"
          ? await UserService.addStoreRole(params)
          : await UserService.updateStoreRole({
            ...params
          })
      if (res.code === 0) {
...
const getDictConfig = async () => {
  const res = await ApiService.getDictConfig("roleDataTypeEnum")
  const data = await ApiService.getDictConfig("roleTypeEnum")
