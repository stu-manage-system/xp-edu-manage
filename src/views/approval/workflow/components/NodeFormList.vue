<template>
  <div style="display: flex; flex-direction: column; gap: 10px" v-loading="loading">
    ...
    >
      <el-col :span="2">
        <el-form-item label="" style="height: 40px">
          <el-tag style="height: 40px" :type="index === 0 ? 'success' : 'warning'">{{
            index === 0 ? "申请节点" : `${index}级审批`
          }}</el-tag>
        </el-form-item>
      </el-col>
      <el-col :span="6">
        <el-form-item label="节点名称" class="flex" required>
          <el-input placeholder="请输入节点名称" v-model="node.nodeName" clearable :disabled="props.disabled" />
        </el-form-item>
      </el-col>
      <el-col :span="6">
        <el-form-item label="类型" class="flex" required>
          <el-select v-model="node.approvalType" :disabled="index === 0 || props.disabled">
            <el-option
    ...
    <el-button v-if="!disabled" type="primary" @click="addNode">新增节点</el-button>
  </div>
</template>
